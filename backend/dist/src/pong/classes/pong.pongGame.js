...
class pongGame {
    constructor(player1, player2, gameRepo, gameUserRepo, server, gameMode) {
    ...
    async createGame() {
        ...
        this.server
            .to(this.room)
            .emit('gameReadyToStart', this.room, this.player1.userName, this.player2.userName, this.gameMode);
    ...
    updateBallDirection() {
        if (this.ballPosition.y <= BALL_RADIUS ||
    ...
    accelerateBall() {
        if (this.ballSpeed * 2 + BALL_ACCELERATION * 2 <
