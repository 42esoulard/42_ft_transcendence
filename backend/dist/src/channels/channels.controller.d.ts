...
export declare class ChannelsController {
    ...
    setNewMessage(status: string, chanId: number, request: Request): Promise<ChannelMember>;
    toggleNotification(cmId: number, request: Request): Promise<ChannelMember>;
    joinChannel(type: string, channel_id: number, user_id: number, request: Request): Promise<ChannelMember>;
    checkPasswordMatch(channel_id: number, attempt: string, request: Request): Promise<boolean>;
    ...
    leaveChannel(type: string, cm_id: number, request: Request): Promise<DeleteResult>;
    deleteChannel(chan_id: number, request: Request): Promise<DeleteResult>;
    muteBanMember(action: string, cm_id: number, end_date: number, request: Request): Promise<ChannelMember>;
    toggleAdmin(cm_id: number, request: Request): Promise<ChannelMember>;
    updateChannelPassword(chan_id: number, pwd: string, request: Request): Promise<Channel>;
}
