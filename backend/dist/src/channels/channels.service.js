...
let ChannelsService = class ChannelsService {
    constructor(channelsRepository, userService, channelMemberService, relationshipService) {
    ...
    async seed() {
        ...
        return await this.channelsRepository
            ...
            .catch(() => {
            throw new common_1.BadRequestException('Channel did not comply database requirements');
        });
    }
    async getChannels() {
        ...
        return await this.relationshipService
            ...
            channels.forEach((chan) => {
                if (chan.messages && res) {
                    chan.messages = chan.messages.filter((msg) => !res.map((res) => res.adresseeId).includes(msg.author.id));
                }
                if (chan.channel_members && res) {
                    chan.channel_members = chan.channel_members.filter((member) => !res.map((res) => res.adresseeId).includes(member.member.id));
                }
            });
            return channels;
        });
    ...
    async getAvailableChannels(user_id) {
        ...
        return await this.getUserChannels(user_id).then(async (res) => {
            ...
            channels = channels.filter((channels) => !userChannels
                .map((userChannels) => userChannels.id)
                .includes(channels.id));
            if (user.role == role_enum_1.Role.OWNER || user.role == role_enum_1.Role.ADMIN) {
        ...
        return await this.channelsRepository
            ...
            return await this.relationshipService
                ...
                if (channel.messages && blocked) {
                    channel.messages = channel.messages.filter((msg) => !blocked
    ...
    async leaveChannel(cm_id) {
        ...
        return await this.channelMemberService
            ...
            .then(async (res) => {
            if (cm.channel.channel_members.length == 1 &&
    ...
    async saveDmChannel(dmInfo) {
        ...
        return await this.getChannelsByName(dmInfo.name).then(async (res) => {
            if (res) {
                res.forEach((chan) => {
                    if (chan.channel_members.length == 2 &&
                        ...
                        throw new common_1.BadRequestException('DM channel already exists!');
                    }
                });
            }
            ...
            return await this.getChannelsByName(altName).then(async (res) => {
                ...
                return await this.checkBlocked(dmInfo.owner_id, dmInfo.recipient_id).then(async (res) => {
                    ...
                    return await this.channelsRepository
                        .save(dmInfo)
                        .then(async (res) => {
                        await this.channelMemberService.createChannelMember(res, recipient, true, true, dmInfo.notification);
                        return await this.channelMemberService.createChannelMember(res, owner, true, true, dmInfo.notification);
                    })
                        .catch(() => {
                        throw new common_1.BadRequestException('Channel did not comply database requirements');
                    });
    ...
    async saveChannel(channelDto) {
        ...
        return await this.getChannelByName(channelDto.name).then(async (res) => {
            ...
            return await this.channelsRepository
                .save(newChannel)
                .then(async (res) => {
                return await this.channelMemberService.createChannelMember(res, owner, true, true, channelDto.notification);
            })
                .catch(() => {
                throw new common_1.BadRequestException('Channel did not comply database requirements');
            });
