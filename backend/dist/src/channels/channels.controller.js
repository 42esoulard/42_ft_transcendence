...
let ChannelsController = class ChannelsController {
    ...
    async getDefaultChannel(request) {
        const channel = await this.channelService.getChannelById(1, request.user.id);
        if (channel == undefined) {
            const generalChan = await this.channelService.seed();
            if (generalChan == undefined) {
                throw new common_1.NotFoundException("Couldn't initialize general channel");
            }
            return generalChan;
        }
        return channel;
    ...
    async getChannelById(chanId, request) {
        const cm = await this.channelService.getChannelMember(chanId, request.user.id);
        if (cm == undefined) {
            throw new common_1.ForbiddenException('You must be a member to access a channel');
        }
        const channel = await this.channelService.getChannelById(chanId, request.user.id);
        if (channel == undefined) {
            if (chanId == 1) {
                const generalChan = await this.channelService.seed();
                if (generalChan == undefined) {
    ...
    async getChannelPreview(chanId, request) {
        const channel = await this.channelService.getChannelPreview(chanId, request.user.id);
        if (channel == undefined) {
            throw new common_1.NotFoundException('Channel not found');
        }
        return channel;
    ...
    async toggleNotification(cmId, request) {
        ...
        if (cm == undefined) {
            ...
            throw new common_1.ForbiddenException('Only subscribed users can change their own notification settings');
        }
    ...
    async joinChannel(type, channel_id, user_id, request) {
        ...
        switch (type) {
            case 'added':
                await this.channelService
                    ...
                    if (res == true && request.user.role == 'user') {
                        throw new common_1.ForbiddenException('Failed to join channel: missing authorization to act for this user: active block');
                    }
                });
                await this.channelService
                    ...
                    if (!res) {
                        ...
                        if (user.role !== role_enum_1.Role.ADMIN && user.role !== role_enum_1.Role.OWNER) {
                            throw new common_1.ForbiddenException('Failed to join channel: missing authorization to act for this user');
                        }
                    }
            ...
            case 'self':
                ...
                break;
            default:
                throw new common_1.BadRequestException('Failed to join channel: bad type for the add request');
        }
    ...
    async checkPasswordMatch(channel_id, attempt, request) {
        const match = await this.channelService.checkPasswordMatch(channel_id, request.user.id, attempt);
        if (match == undefined) {
            throw new common_1.NotFoundException("Couldn't attempt to match password");
        }
        return match;
    }
    ...
    async getAvailableChannels(request) {
        const channels = await this.channelService.getAvailableChannels(request.user.id);
        if (channels == undefined) {
    ...
    async leaveChannel(type, cm_id, request) {
        ...
        switch (type) {
            case 'kick':
                ...
                await this.channelService
                    ...
                    if (!res) {
                        const user = await this.channelService.getUser(request.user.id);
                        if (user == undefined) {
                            throw new common_1.NotFoundException("Couldn't identify request account");
                        }
                        if (user.role !== role_enum_1.Role.ADMIN && user.role !== role_enum_1.Role.OWNER) {
                            throw new common_1.ForbiddenException('Failed to leave channel: missing authorization to act for this user');
                        }
                    }
                    else if (!res.is_admin &&
                        !res.is_owner &&
            ...
            case 'self':
                ...
                break;
            default:
                throw new common_1.BadRequestException('Failed to leave channel: bad type for the leave request');
        }
    ...
    async deleteChannel(chan_id, request) {
        if (chan_id !== 1) {
            const cm = await this.channelService.getChannelMember(chan_id, request.user.id);
            if (cm == undefined) {
                const user = await this.channelService.getUser(request.user.id);
                if (user == undefined) {
                    throw new common_1.NotFoundException("Couldn't identify request account");
                ...
                if (user.role !== role_enum_1.Role.ADMIN && user.role !== role_enum_1.Role.OWNER) {
                    throw new common_1.ForbiddenException('You dont have the right to delete this channel');
                }
            }
            ...
            throw new common_1.ForbiddenException("General can't be deleted");
    }
    async muteBanMember(action, cm_id, end_date, request) {
        ...
        if (req_cm == undefined) {
            ...
            if (user.role !== role_enum_1.Role.ADMIN && user.role !== role_enum_1.Role.OWNER) {
                throw new common_1.ForbiddenException("You don't have the right to act on this channels members");
            }
        }
        else if (!req_cm.is_admin &&
            !req_cm.is_owner &&
        ...
        if (cm == undefined) {
            throw new common_1.NotFoundException("Failed to edit member's mute/ban status");
        }
        return cm;
    }
    async toggleAdmin(cm_id, request) {
        ...
        if (req_cm == undefined) {
            ...
            if (user.role !== role_enum_1.Role.ADMIN && user.role !== role_enum_1.Role.OWNER) {
                throw new common_1.ForbiddenException('You dont have the right to act on this channels members');
            }
        }
        ...
        if (cm == undefined) {
            throw new common_1.NotFoundException("Failed to edit member's admin status");
        }
        return cm;
    }
    async updateChannelPassword(chan_id, pwd, request) {
        const req_cm = await this.channelService.getChannelMember(chan_id, request.user.id);
        if (req_cm == undefined) {
            const user = await this.channelService.getUser(request.user.id);
            if (user == undefined) {
                throw new common_1.NotFoundException("Couldn't identify request account");
        ...
        else if (!req_cm.is_owner && req_cm.member.role == role_enum_1.Role.USER) {
            throw new common_1.ForbiddenException('You dont have the right to edit this channel');
        }
        const channel = await this.channelService.updateChannelPassword(chan_id, request.user.id, pwd);
        if (channel == undefined) {
...
__decorate([
    ...
    (0, common_1.UseGuards)(jwtTwoFactor_guard_1.JwtTwoFactorGuard, roles_guard_1.RolesGuard),
    openapi.ApiResponse({ status: 200, type: [require("./interfaces/channel.interface").Channel] }),
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "getChannelsNames", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "getDefaultChannel", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "getChannelById", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Number, Object]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "getChannelPreview", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "getChannelByName", null);
__decorate([
    (0, common_1.Post)(),
    (0, common_1.UseGuards)(jwtTwoFactor_guard_1.JwtTwoFactorGuard),
    openapi.ApiResponse({ status: 201, type: require("../channel_members/interfaces/channel_member.interface").ChannelMember }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [createChannel_dto_1.CreateChannelDto]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "saveChannel", null);
__decorate([
    (0, common_1.Post)('/dm'),
    (0, common_1.UseGuards)(jwtTwoFactor_guard_1.JwtTwoFactorGuard),
    openapi.ApiResponse({ status: 201, type: require("../channel_members/interfaces/channel_member.interface").ChannelMember }),
    ...
    __metadata("design:paramtypes", [createChannel_dto_1.CreateChannelDto]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "saveDmChannel", null);
__decorate([
    (0, common_1.Get)('/chat-message/:status/:chanId'),
    (0, common_1.UseGuards)(jwtTwoFactor_guard_1.JwtTwoFactorGuard),
    openapi.ApiResponse({ status: 200, type: require("../channel_members/interfaces/channel_member.interface").ChannelMember }),
    ...
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "setNewMessage", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Number, Object]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "toggleNotification", null);
__decorate([
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "checkPasswordMatch", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "getUserChannels", null);
__decorate([
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "getChannelMember", null);
__decorate([
...
__decorate([
    ...
    __metadata("design:paramtypes", [String, Number, Number, Object]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "muteBanMember", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Number, Object]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "toggleAdmin", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Number, String, Object]),
    __metadata("design:returntype", Promise)
], ChannelsController.prototype, "updateChannelPassword", null);
