...
let UsersService = class UsersService {
    ...
    async getAdmins() {
        return await this.usersRepository.find({
            where: [{ role: role_enum_1.Role.ADMIN }, { role: role_enum_1.Role.OWNER }],
        });
    }
    async getOwner() {
        return await this.usersRepository.find({ where: { role: role_enum_1.Role.OWNER } });
    }
    ...
    async changeOwner(id) {
        ...
        const user = await this.usersRepository.findOne(id);
        await this.usersRepository.update(owner[0].id == id ? owner[1].id : owner[0].id, { role: role_enum_1.Role.ADMIN });
        await this.usersRepository.update(user.id, { role: role_enum_1.Role.OWNER });
    }
