...
let RelationshipsService = class RelationshipsService {
    ...
    async saveRelationship(relationshipDto) {
        if (relationshipDto.nature == 'blocked') {
            const relationship = await this.getRelationship(relationshipDto.adresseeId, relationshipDto.requesterId);
            if (relationship)
        ...
        const newRelationship = this.relationshipsRepository.create(relationshipDto);
        if (relationshipDto.nature != 'blocked')
    ...
    async removeRelationship(relationshipDto) {
        const relationship = await this.getRelationship(relationshipDto.userId1, relationshipDto.userId2);
        return await this.relationshipsRepository.delete(relationship.id);
    }
};
RelationshipsService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(relationships_entity_1.Relationships)),
